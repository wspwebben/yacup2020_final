...
const RED_MULTIPLIER = 0.21;
const GREEN_MULTIPLIER = 0.72;
const BLUE_MULTIPLIER = 0.07;

function getGray(r, g, b) {
  return RED_MULTIPLIER * r + GREEN_MULTIPLIER * g + BLUE_MULTIPLIER * b;
}

function convertFrameIntoGray(frame) {
  const grayFrame = new Uint8ClampedArray(frame.length / 4)
  ...
  for (let index = 0; index < frame.length; index += 1) {
    ...
    // const alpha = frame[sourceIndex + 3];
    
    const gray = getGray(red, green, blue);
...
function dilateFrame(frame) {
  return frame;
}

function eroseFrame(frame) {
  return frame;
}

module.exports = async function (sourceFrame, size) {
  const frame = convertFrameIntoGray(sourceFrame);

  const morph = new Morph(size.height, size.width, frame);

  morph.erodeWithElement();
  // morph.dilateWithElement(el)
